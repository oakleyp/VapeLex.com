...
if(count($cart_data) > 0 && isset($_GET['submit'])) {
	...
	do {
	 $params = USPSParcelRate(1, $_POST['zipcode'], $_POST['select-service']);
	} while(isset($params['ERROR']));
	if($service == "FIRST CLASS") {
	$shipping = $params['RATEV4RESPONSE']['0']['0']['RATE'];
	} if($service == "PRIORITY") {
	$shipping = $params['RATEV4RESPONSE']['0']['1']['RATE'];
...
//echo $params['RATEV4RESPONSE']['0']['3']['RATE'];
	...
	$tapi_login_id = 'xxxx';
	$ttransaction_key = 'xxxx';
	$amount = $total;
	$fp_timestamp = time();
	$fp_sequence = $order_id; // Enter an invoice or other unique number.
	$fingerprint = AuthorizeNetSIM_Form::getFingerprint($tapi_login_id,
  	$ttransaction_key, $amount, $fp_sequence, $fp_timestamp);
	$item_fields = "";
	foreach($cart_data as $item) {
		$item_attr = $cart->getItemData($item[0]);
		$item_desc = str_replace(';', ' ', $item[3]);
		$item_fields .= '<input type="hidden" name="x_line_item" value="item'.$item[0].'<|>'.$item_attr['name'].'<|>'.$item_desc.'<|>'.$item[1].'<|>'.$cart->getItemPrice($item[0], $item[3]).'<|>N" />';	
	}
	if($discount>0) { 
	...
	$formsubstr = '
    <FORM METHOD="POST" ACTION="https://test.authorize.net/gateway/transact.dll" name="_xclick"> 
	<input type="hidden" name="x_login" value="'.$tapi_login_id.'" />
        		...
        		<div class="row">
        			<div class="col-md-12">
						<header class="content-title">
							<h1 class="title">Redirecting you to our secure third-party payment processor...						</h1>
						</header>
       				  <div class="xs-margin"></div><!-- space -->
                      	<img style="text-align:center; vertical-align:middle" src="images/load.gif" />
                        <?php echo($formsubstr); ?>
                    </div>
                </div>
            </div>
							   		  ...
							   		  <div class="xs-margin"></div>
								   		<div class="input-group sm-margin">
											 <select name="select-service" size=5 style="color:#000000;">
                                             	<option id="pexpress" value="PRIORITY MAIL EXPRESS">Priority Express Overnight (Lexington Only) - ~$19.99</option>
                                                <option id="pstandard" value="PRIORITY">Priority Mail - ~$6.00</option>
                                                <option id="firstclass" value="FIRST CLASS">First-Class Mail - ~$2.00</option>
                                             </select>
        					...
        					<div class="panel">
								...
								<tbody>
									...
									$cookie_data = $cart->getCartItems();
										...
										<td>
												<span id="quantity'. $cart_item[0] .'" name="quantity'. $cart_item[0] .'">'. $cart_item[1] .'</span>
										</td>
